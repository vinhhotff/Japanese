...
const Dashboard = () => {
  ...
  const loadCourses = async () => {
    try {
      ...
      // Separate Japanese and Chinese courses
      ...
      // Group Japanese courses
      const japaneseGrouped = japaneseLevels
      ...
      const chineseGrouped = chineseLevels
        .filter(level => coursesByLevel[level] && coursesByLevel[level].length > 0)
        .map(level => {
          const allLessons = lessonsData.filter(l => {
            const courseId = l.course_id || l.course?.id;
    ...
    <div className="container">
        ...
        <div style={{
          ...
          <div className="card" style={{ maxWidth: '500px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '1.125rem', marginBottom: '2rem', lineHeight: '1.6' }}>
              Bắt đầu hành trình học ngôn ngữ của bạn ngay hôm nay!
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '2rem', marginBottom: '0.25rem', color: 'white', fontWeight: '800', lineHeight: '1.2' }}>
                    Học Tiếng Nhật & Tiếng Trung
                  </h1>
                  <p style={{ fontSize: '1rem', opacity: 0.9, margin: 0 }}>
                    Học dễ dàng, hiệu quả với AI
                  </p>
                </div>
              </div>
      ...
      {chineseCourses.length > 0 && (
        <div style={{ marginBottom: '3rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
            {chineseCourses.map((course, index) => (
              <Link key={course.level} to={`/chinese/courses/${course.level}`} style={{ textDecoration: 'none' }}>
                <div style={{ 
                  height: '100%',
                  transition: 'all 0.3s',
                  cursor: 'pointer',
